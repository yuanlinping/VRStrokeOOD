...
class mydataset(object):
    ...
    def __getitem__(self, index):
        ...
        elif self.task == 'stroke_data':
            ctarget = self.target_trans(self.labels[index])
            dtarget = self.target_trans(self.dlabels[index])
            pctarget = self.target_trans(self.pclabels[index])
            pdtarget = self.target_trans(self.pdlabels[index])
            values = self.target_trans(self.values[index])
            return x, ctarget, dtarget, pctarget, pdtarget, values, index
...
class subdataset(mydataset):
    def __init__(self, args, dataset, indices):
        ...
        self.values = dataset.values[indices]
...
class combindataset(mydataset):
    def __init__(self, args, datalist):
        ...
        zlist = [item.z for item in datalist]
        cylist = [item.labels for item in datalist]#类标签（0-5）
        dylist = [item.dlabels for item in datalist]#域标签(4个域：-1，0，1，2)
        ...
        self.values = torch.vstack(zlist)
